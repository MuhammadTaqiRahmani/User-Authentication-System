...
const fs = require('fs');
const bodyParser = require('body-parser');
const { Connection, Request, TYPES } = require('tedious'); // Import TYPES
require('dotenv').config(); // Load environment variables
  ...
  const checkRequest = new Request(checkEmailQuery, (err, rowCount, rows) => {
    if (err) {
      ...
      if (emailExists) {
        ...
        insertRequest.addParameter('Password', TYPES.NVarChar, password);
        connection.execSql(insertRequest);
      }
    }
  });
...
app.post('/signin', (req, res) => {
    ...
    const request = new Request(query, (err, rowCount, rows) => {
      if (err) {
        ...
        if (rows.length > 0) {
          const count = rows[0][0].value; // Access the 'count' column value correctly
          if (count > 0) {
            userFound = true;
          }
        }
