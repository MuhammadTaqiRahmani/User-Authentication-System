...
app.post('/signup', (req, res) => {
    ...
    const checkRequest = new Request(checkEmailQuery, (err, rowCount, rows) => {
        if (err) {
            ...
            console.log(`Signup - rowCount: ${rowCount}`); // Log rowCount to check its value
            console.log(`Signup - rows: ${JSON.stringify(rows)}`); // Log rows to see the returned data
...
app.post('/signin', (req, res) => {
    ...
    const loginRequest = new Request(checkLoginQuery, (err, rowCount, rows) => {
        ...
        // Log rowCount and rows to debug
        console.log(`rowCount: ${rowCount}`);
        console.log(`rows: ${JSON.stringify(rows)}`);

        if (rowCount > 0 && rows.length > 0 && rows[0].length > 0) {
            const storedHashedPassword = rows[0][0].value;
